(() => {
  ...
  function renderSpendingChart() {
    ...
    chartWeeks=C.spendingWeeks(filteredWeeklyEntries(),chartWeeks.at(-1).from)
    const selection=data.dashboardExpenseCategories||{},ids=[...new Set([...state.categories.map(c=>c.id),...state.entries.filter(e=>!e.deleted&&e.type==='expense').map(e=>e.categoryId)])]
    const categoryNames=new Map(state.categories.map(c=>[c.id,c.name]))
    $('financeExpenseFilters').innerHTML=ids.map(id=>`<label><input type="checkbox" data-expense-category="${escape(id)}" ${selection[id]===false?'':'checked'}><span>${escape(categoryNames.get(id)||'카테고리 없음')}</span></label>`).join('')||'<span class="feature-help">지출 카테고리가 없어요.</span>'
